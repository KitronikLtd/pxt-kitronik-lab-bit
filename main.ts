...
namespace kitronik_lab_bit {
    ...
    function measure(units: number, maxCmDistance = 500): number {
        ...
        while(measure == 0) {
            ...
            switch (units) {
                case CM_UNITS: measure = Math.idiv(pulse, cmEquationDivider); break;
                case IN_UNITS: measure = Math.idiv(pulse, inEquationDivider); break;
                //default: return 0;
            }
        }
        return measure;
    ...
    /**
     * Measure the echo time (after trigger) and converts to cm, then return value as an int
     */
    //% subcategory="Inputs"
    //% group="Ultrasonic"
    ...
    /**
     * Measure the echo time (after trigger) and converts to inches, then return value as an int
     */
    //% subcategory="Inputs"
    //% group="Ultrasonic"
